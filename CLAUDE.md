...
`.github/workflows/ci.yml` runs lint (ESLint + CSS lint as two steps in the same job), typecheck, Jest+coverage, and a production build as separate jobs on PRs/pushes to `main`/`develop`. `.github/workflows/playwright.yml` runs the E2E suite. Both have a `changes` job (using `dorny/paths-filter`) that the other jobs `need` and gate on via `if: needs.changes.outputs.code == 'true'` - when every changed file is a `.md`/`.MD` file or under `reference/**`, the lint/typecheck/test/build/playwright jobs still run (so they still report a status) but their steps are skipped, showing as "skipped" rather than executing. This is deliberately job-level, not workflow-trigger-level (e.g. `paths-ignore` on `on:`): `main` has required status checks for these exact job names, and a workflow that never triggers at all never reports a status for them, which leaves a PR stuck with checks "waiting to be reported" forever - `dorny/paths-filter` is GitHub's documented workaround for that interaction. `.github/workflows/validate-manifest.yml` regenerates/validates the photo manifest. None of these gate the actual Vercel deploy (git-push-to-deploy from `main`).
