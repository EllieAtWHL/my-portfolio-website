...
`.github/workflows/ci.yml` runs lint (ESLint + CSS lint as two steps in the same job), typecheck, Jest+coverage, and a production build as separate jobs on PRs/pushes to `main`/`develop`. `.github/workflows/playwright.yml` runs the E2E suite. Both have a `changes` job that the other jobs `need`, computing whether any non-doc file changed via a plain `git diff --name-only` between the PR's merge-base and head (or the previous/current commit for direct pushes) - `code=false` only if every changed file is a `.md`/`.MD` file, under `reference/**`, or exactly `public/spurs-women/photo-gallery.manifest.json` (the generated photo manifest - a change to it alone can't affect lint/typecheck/test/build/e2e outcomes any more than a docs change can, so the automated `auto/update-manifest-*` PRs skip the full suite the same way doc-only PRs do; `.github/workflows/validate-manifest.yml`, the check that actually validates this file's shape, still runs unconditionally). In `ci.yml` this gates each job as a whole (`if: needs.changes.outputs.code == 'true'` on the job), so `lint`/`typecheck`/`test`/`build` still run (reporting "skipped") but do nothing for doc-only changes. `playwright.yml`'s `playwright` job is a matrix (`chromium`/`firefox`/`webkit`) and deliberately does **not** use a job-level `if:` - GitHub only expands a matrix into its per-value check names (`playwright (chromium)` etc.) once the job actually starts, so a job-level `if: false` produces a single unexpanded `playwright (${{ matrix.browser }})` check instead of the three required ones, leaving them stuck exactly like the workflow-trigger-level problem below. Gating each *step* instead keeps the matrix expanding normally; the three named jobs then report "success" with all steps skipped. Both workflows are deliberately job/step-level, not workflow-trigger-level (e.g. `paths-ignore` on `on:`): `main` has required status checks for these exact job names, and a workflow that never triggers at all never reports a status for them, which leaves a PR stuck with checks "waiting to be reported" forever. (An earlier version of the diff used the third-party `dorny/paths-filter` action - dropped after its default negated-glob-list semantics turned out not to mean what they looked like, silently running the full suite on every PR; the hand-rolled `git diff` + `grep` is simpler to audit and was verified against `gh pr diff`'s actual output before being trusted.) `.github/workflows/validate-manifest.yml` regenerates/validates the photo manifest. None of these gate the actual Vercel deploy (git-push-to-deploy from `main`).
