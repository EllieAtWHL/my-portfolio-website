...
Business-logic-heavy code (the Supabase data-access layer in `src/lib/data/`, hooks with real state machines like `useRegicideGame`, security-relevant code like the admin-route auth guard in `src/lib/supabase/middleware.ts`, parsing/formatting utilities) gets deep, mutation-tested unit tests. Purely presentational components and pages (static content pages, icon wrappers, layout shells) are lower-value mutation-testing targets - a wrong CSS class rarely has a "correct" assertion worth defending at the unit level - and are covered more cheaply via the Playwright E2E suite plus lighter render/smoke tests where warranted. As of 2026-09 this split is: the logic layer (`src/lib`, `src/lib/data`, `src/hooks`) sits at 90-100% *statement* coverage with mutation-tested assertions - branch coverage in the same layer trails a bit behind statements (`src/hooks` in particular is ~77% on branches, vs. 91.68% statements), since not every conditional has a mutation-tested case for both branches yet; most presentational components/pages are still untested at the unit level and are the target of a planned follow-up pass (see `git log` / recent PRs for current status - this file describes the target methodology, not a point-in-time task list).
...
Run `npm run test:coverage` for the current numbers - `coverageThreshold` in `jest.config.js` documents the last-measured baseline inline and is kept below it so a real regression fails the check without being fragile to normal fluctuation.

**Why the threshold numbers dropped in 2026-08, and why that's not a regression:** before then, `jest.config.js` had no `collectCoverageFrom`, so coverage was only measured over the ~25% of the codebase some test happened to `import` - the other ~150 files (mostly pages/components) were invisible to the report rather than counted as 0%, which is how the old thresholds reached 75/60/70/75. Adding `collectCoverageFrom` made the same test suite's *honest* whole-app number ~31%. Thresholds were reset below that honest baseline and have been rising since as real tests were added (31% → 54.5% → 58.5% → 61.7% → 62.8% → 67.0% → 69.74% statements, tracking the `lib`/`lib/data`/`hooks` and then `hooks/admin` mutation-testing passes, plus the 2026-08-26 WEB-62/WEB-124–129 component-split work and the WEB-125 error-boundary coverage and WEB-169 `RelatedList`/`getTeamDisplayName` tests added since). The numeric threshold is lower than it used to be; actual coverage is unchanged or higher. As of this writing (2026-09), `npm run test:coverage` reports 132 test suites / 1212 tests passing, at 69.74% statements / 63.5% branches / 66.31% functions / 69.92% lines - `coverageThreshold` in `jest.config.js` (54/48/48/54) sits well below that (15-18 points, not "a few"), per the policy above; re-run `npm run test:coverage` periodically to catch this line drifting stale again, as it has four times before now.
