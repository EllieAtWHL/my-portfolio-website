...
- `src/components/ErrorBoundary.tsx` ("Try again" reset button)
...
- `src/components/admin/CollapsibleFormSection.tsx` (collapsible section toggle, extracted from `MatchForm.tsx` — `MatchForm.tsx` itself no longer renders a raw `<button>`, it now composes this shared component)
