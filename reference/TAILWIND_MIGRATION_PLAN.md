...
- The rest of `globals.css`'s own inline rules (the bulk of the file, beyond the heading/link rules) are still unlayered. No confirmed collision was found there (unlike bare `h1`-`h4`/`a`, everything else in the file is class-scoped, so normal specificity applies regardless of layering) - flagged as a lower-priority hygiene gap, not a live bug.
- `spurs-theme.css` (loaded via a JS `import` in `spurs-women/layout.tsx`, not a CSS `@import`, so `layer()` doesn't apply the same way) is also still unlayered, for the same reason - no confirmed collision found inside it.
