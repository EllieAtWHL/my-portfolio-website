...
// on the London 2012 blog, OpenStreetMap on Spurs Women stadium pages, Bluesky post
// embeds on Spurs Women match media) plus the hosts VideoGrid calls client-side for
// oEmbed metadata (src/lib/youtube.ts), Supabase (auth + data), FullStory
// (public/fullstory-init.js) and Vercel Analytics (@vercel/analytics/next, which
// loads its script from va.vercel-scripts.com rather than same-origin). img-src is
// left broad (https:) because the portfolio side of the site links out to a wide,
// one-off set of external preview images (see OG `images` arrays across
// src/app/**/page.tsx) that aren't worth enumerating host-by-host.
...
// WEB-70 PR description for the checklist. Bluesky's embed.bsky.app script-src
// entry added under WEB-146 - it was missing entirely, silently blocking every
// Bluesky post embedded as match media.
const CSP_DIRECTIVES = [
  "default-src 'self'",
  `script-src ${scriptSrc.join(" ")} https://edge.fullstory.com https://va.vercel-scripts.com https://embed.bsky.app`,
  "style-src 'self' 'unsafe-inline'",
  "img-src 'self' data: blob: https:",
  "font-src 'self' data:",
  "connect-src 'self' https://*.supabase.co https://www.youtube.com https://noembed.com https://*.fullstory.com",
  "frame-src https://www.youtube.com https://open.spotify.com https://www.openstreetmap.org https://embed.bsky.app",
  "object-src 'none'",
  "base-uri 'self'",
  "form-action 'self'",
